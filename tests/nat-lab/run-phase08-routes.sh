...
cleanup_route_lab() {
  clear_phase06_drop_rules >/dev/null 2>&1 || true
  ssh_to_vm client-a "sudo iptables -D FORWARD -i ${MESHLINK_INTERFACE_NAME} -o ${ROUTE_HOST_IF} -d ${ROUTED_SUBNET} -j ACCEPT 2>/dev/null || true; sudo iptables -D FORWARD -i ${ROUTE_HOST_IF} -o ${MESHLINK_INTERFACE_NAME} -s ${ROUTED_SUBNET} -j ACCEPT 2>/dev/null || true; sudo ip link del ${ROUTE_HOST_IF} 2>/dev/null || true; sudo ip netns del ${ROUTE_NAMESPACE} 2>/dev/null || true" >/dev/null 2>&1 || true
}
...
EOF
  ...
  if [[ -n "$advertised_routes" ]]; then
    printf 'advertised_routes = [%s]\n' "$advertised_routes" >>"$MESHLINK_LAB_STATE_DIR/runtime/config/${node}.toml"
  fi
}
...
start_client() {
  local node="$1"
  ssh_to_vm "$node" "sudo pkill -x meshlinkd || true; sudo ip link del ${MESHLINK_INTERFACE_NAME} 2>/dev/null || true; nohup sudo timeout 150s ${REMOTE_ROOT}/bin/meshlinkd --config ${REMOTE_ROOT}/config/client.toml > ${REMOTE_ROOT}/logs/meshlinkd.log 2>&1 < /dev/null &"
}
...
wait_for_router_tools() {
  ...
  while (( attempt <= attempts )); do
    if ssh_to_vm "$node" "command -v iptables >/dev/null 2>&1 && command -v ip >/dev/null 2>&1 && sudo true" >/dev/null 2>&1; then
      return 0
    fi
    sleep 2
    attempt=$((attempt + 1))
...
wait_for_peer_route() {
  ...
  while (( attempt <= attempts )); do
    if ssh_to_vm "$node" "sudo ip route show dev ${MESHLINK_INTERFACE_NAME} | grep -q '^${route} '" >/dev/null 2>&1; then
  ...
  echo "timed out waiting for route ${route} to become ${expected_state} on ${node}" >&2
  return 1
}
...
build_artifacts
generate_wireguard_keys
write_client_config client-a "$CLIENT_A_PUBLIC_KEY" "$CLIENT_A_PRIVATE_KEY" "$MESHLINK_CLIENT_A_WG_PORT" "\"${ROUTED_SUBNET}\""
...
wait_for_remote_log mgmt-1 "${REMOTE_ROOT}/logs/managementd.log" "managementd gRPC listening on 0.0.0.0:${MESHLINK_MANAGEMENT_PORT}"
